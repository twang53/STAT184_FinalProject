...
- join on language, total speakers for each state


(C) .Rmd Line number(s) for a spread or gather operation (or equivalent):   
- can plot diff lines for languages based on total speakers (right now is long, to go to wide key = language, value = total speakers?)


(D) .Rmd Line number(s) for use of regular expressions:   
- detect patterns using regex grep or grepl (langauges with spaces, ends w/ number, starts or ends w/ a certain pattern)
...
- print leaflet map for each state?
